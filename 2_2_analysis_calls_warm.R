...
###################################################################################
###################################################################################
## overall : total calls (may-oct warm) (feeds into Figures 10  and 11)
###################################################################################
###################################################################################
...
pred_tmax <- crosspred(cb.tmax.obs, mod_tmax, cen=19, by=1)
pred_tmin <- crosspred(cb.tmin.obs, mod_tmin, cen=8, by=1)
pred_himax <- crosspred(cb.himax.obs, mod_himax, cen=19, by=1)
pred_himin <- crosspred(cb.himin.obs, mod_himin, cen=8, by=1)
...
res <- rbind(cbind(df_tmax, expo="Maximum Temperature (ref=19C)", ref=19),
             cbind(df_tmin, expo="Minimum Temperature (ref=8C)", ref=8),
             cbind(df_himax, expo="Maximum Heat Index (ref=19C)", ref=19),
             cbind(df_himin, expo="Minimum Heat Index (ref=8C)", ref=8))


p_type <- res %>%
  mutate(m=ifelse(ref==8, "Minimum", "Maximum"),
         t_hi=ifelse(expo %in% c("Maximum Temperature (ref=19C)", "Minimum Temperature (ref=8C)"), "Temperature", "Heat index")) %>%
  ggplot(aes(group=t_hi, color=t_hi)) +
  geom_hline(yintercept=1, linetype="dashed", color = "grey", linewidth=0.8) +
  labs(title = "Police emergency calls: warm months", x="Celsius", y="Relative risk") +
...
p_type <- res %>%
  ...
  labs(title = "Police emergency calls: warm months", x="Celsius", y="Count Difference") +
  geom_ribbon(aes(x=deg, ymin=lo, ymax=hi, fill=t_hi), alpha=0.1, color=NA) +
  geom_line(aes(x=deg, y=est)) +
  scale_y_continuous(
    "Count difference", 
