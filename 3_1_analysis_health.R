...
process_rr <- function(cross_pred){
  ...
  df_res <- df_est %>%
    left_join(df_lo, by = join_by(deg, lag)) %>%
    left_join(df_hi, by = join_by(deg, lag)) %>%
...
reason_group <- c("health_mental", "health_non")
lab_group <- c("Mental health incidents", "Other health incidents")
